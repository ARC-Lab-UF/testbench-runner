...
def run_simulations(students: List[StudentData]):
    total_num_students = len(students)

    for i, student in enumerate(students, start=1):
        # Has this student already been simulated at least once before?
        ran_once = False
        ...
        while True:
            ...
            if ran_once:
                ...
                )  # Technically any unused key works too
                print("- Press g to rerun simulation in the ModelSim GUI")
            else:
                print(
                    "- Press return to run simuation"
                )  # Technically any unused key works too
            ...
            if choice == "q":
                ...
                print(
                    '- If prompted with "A project of this name already exists. Do you want to overwrite it?", select "Yes".'
                )
                print(
                    '- To spend more time in the GUI, select "No" when prompted to quit. The grader will continue to new students whenever you choose to quit the GUI.\n'
                )

            # Create the command to run modelsim
            # Run modelsim (-l "" disables ModelSim logging)
            cmd = (
                lambda gui, do_file: f"vsim {'-gui' if gui else '-c'} -l \"\" -do \"{do_file}\""
            )

            # Run the simulation
            subprocess.run(cmd(gui, student.sim_script), shell=True, stdout=True)
